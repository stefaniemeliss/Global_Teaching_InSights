...
In order to decrease the cognitive demand of rating and improve accuracy, each lesson was segmented into 16-minute segments. A description of how lessons were segmented when the lesson’s length was not a multiple of 16 minutes is provided in Annex A.2. Each of the 24 component codes were rated every 16 minutes by two randomly assigned raters. Each rater watched an entire lesson. Raters were not assigned to rate the same teacher more than once (within or across protocols) for their main study ratings.  
...
Additionally, means and standard errors for each video componet score were computed separately for each country and plotted below.  
...
plt <- ggplot(data = out, aes(x = comp_no, y = mean, fill = group1)) +
  ...
  xlab("Component") + ylab("Average VCOMP score") +
  labs(caption = "Error bars represent SE.") + theme(plot.caption = element_text(hjust=0)) +
...
corrplot(out, 
         ...
         tl.col = "black", number.cex = 1.5, tl.cex = 1.5, cl.cex = 1.5) 
