---
title: "Exploratory Factor Analysis for student-reported measures of teaching-related constructs [SUBSET OF ITEMS for CM]"
author: "Stef Meliss"
date: "`r Sys.Date()`"
...
All items that were used to measure these 12 constructs were included for into an EFA. Please note that classroom management was measured with in total 10 items (SQB11a-j) but the authors only used a subset of these items to derive the subscales disruption and teacher classroom management. **Here, only the subset of items in the classroom management scale were included.** Where appropriate, items were recoded so that a higher item score represent more quality of teaching.   
...
# recode items
df_efa[,paste0("SQB11", LETTERS[c(1, 2, 3)])] <- 5 - df_efa[,paste0("SQB11", LETTERS[c(1, 2, 3)])]
...
Following recoding, a scree plot was generated based on polychoric correlations given the ordinal nature of the data. Applying a cut-off value of 1 to the eigenvalues would result in the recommendation to extract 4-5 factors.
