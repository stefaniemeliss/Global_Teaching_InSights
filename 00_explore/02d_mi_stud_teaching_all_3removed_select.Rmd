---
title: "Measurement invariance (MI): Pupil perspective on teaching-related constructs (selected 5 countries)"
author: "Stef Meliss"
date: "`r Sys.Date()`"
...
all_data$SQB06L_orig <- all_data$SQB06L # copy
all_data$SQB06L <- 5 - all_data$SQB06L # overwrite
...
vars <- c(
  ...
  "USECOGACT_baseline",
  "USECOGACT_posttest",
  # Experience of autonomy, competence, social relatedness
  "USESELFDET_baseline",
  "USESELFDET_posttest",
  # Time on task
  "USETOT_baseline",
...
scales <- c(
  # CM disruption
  paste0("SQA20", LETTERS[1:3],collapse = " + "),
  paste0("SQB11", LETTERS[1:3],collapse = " + "),
  # CM teacher management
  paste0("SQA20", LETTERS[c(6, 7, 9, 10)],collapse = " + "),
  paste0("SQB11", LETTERS[c(6, 7, 9, 10)],collapse = " + "),
  # SE teacher support
  paste0("SQA21", LETTERS[1:3],collapse = " + "),
  paste0("SQB12", LETTERS[1:3],collapse = " + "),
  # SE support competence
  paste0("SQA21", LETTERS[4:7],collapse = " + "),
  paste0("SQB12", LETTERS[4:7],collapse = " + "),
  # SE support autonomy
  paste0("SQA21", LETTERS[8:11],collapse = " + "),
  paste0("SQB12", LETTERS[8:11],collapse = " + "),
  # student teacher relationship 
  paste0("SQA22", LETTERS[1:5],collapse = " + "),
  paste0("SQB13", LETTERS[1:5],collapse = " + "),
  # discourse
  paste0("SQA18", LETTERS[9:11],collapse = " + "),
  paste0("SQB08", LETTERS[9:11],collapse = " + "),
  # OTL
  paste0("SQA16", LETTERS[1:10],"1", collapse = " + "),
  paste0("SQB07E", LETTERS[1:11], collapse = " + "),
  # clarity
  paste0("SQA18", LETTERS[1:4],collapse = " + "),
  paste0("SQB08", LETTERS[1:4],collapse = " + "),
  # meaning
  paste0("SQB09", LETTERS[1:4],collapse = " + "),
  # cognitive activation
  paste0("SQA18", LETTERS[5:8],collapse = " + "),
  paste0("SQB08", LETTERS[5:8],collapse = " + "),
  # adaptation of instruction
  paste0("SQA19", LETTERS[1:5],collapse = " + "),
  paste0("SQB10", LETTERS[1:5],collapse = " + "),
  # feedback
  paste0("SQB16", LETTERS[1:4],collapse = " + "),
  # high expectations
  paste0("SQB14", LETTERS[1:4],collapse = " + "),
  # teacher enthusiasm
  paste0("SQB17", LETTERS[1:8],collapse = " + "),
  # Content-related activities
  paste0("SQA17", LETTERS[1:3],collapse = " + "),
  paste0("SQB06", LETTERS[1:3],collapse = " + "),
  # Self-reported cognitive engagement
  paste0("SQA17", LETTERS[4:6],collapse = " + "),
  paste0("SQB06", LETTERS[4:6],collapse = " + "),
  # Experience of autonomy, competence, social relatedness
  paste0("SQA17", LETTERS[7:9],collapse = " + "),
  paste0("SQB06", LETTERS[7:9],collapse = " + "),
  # Time on task
  paste0("SQA17", LETTERS[10:12],collapse = " + "),
  paste0("SQB06", LETTERS[10:12],collapse = " + ")
)
...
for (v in 1:length(vars)) {
  ...
  # fit CFA
  fit <-lavaan::cfa(model, ordered = T, missing = "pairwise", warn = FALSE, data = stud) 
  ...
  for (c in 1:length(countries)) {
    ...
    tmp_fit <-lavaan::cfa(model, ordered = T, missing = "pairwise", warn = FALSE, data = stud[stud$COUNTRY == countries[c], ])
  ...
  scalar <-lavaan::cfa(model, ordered = T, missing = "pairwise", warn = FALSE, data = stud, group = "COUNTRY", group.equal = c("loadings", "intercepts"))
  ...
  # add to summary
  if (!grepl("DISRUPR_|TESUP_|DISCOURSE_|USECONT_|USECOGACT_|USESELFDET_|USETOT_", vars[v])) {
...
countries_excl <- c("Shanghai", "Japan", "Madrid")
...
# export summary
summary$excluded <- paste0(countries_excl, collapse = "|")
...
# countries to exclude
countries_excl <- c("Shanghai", "Japan", "Chile")
...
# countries to exclude
countries_excl <- c("Shanghai", "Japan", "Mexico")
...
check$excluded <- factor(check$excluded, levels = c("Shanghai|Japan|Madrid", "Shanghai|Japan|Chile", "Shanghai|Japan|Mexico", "Japan|Madrid|Mexico"))
