...
[Artefact component scores (ACOMP)](#ACOMP)  
...
# read in data dict
tmp <- read.delim(file.path(dir, "misc","teacher_file_variables_vcomp_explanation.txt"), header = T)
...
```{r, echo = F, results='asis', fig.align='center', warning=FALSE}
# read in data dict
tmp <- read.delim(file.path(dir, "misc","teacher_file_variables_vind_explanation.txt"), header = T)
...
plt <- ggplot(data = out, aes(x = group1, y = mean, fill = group1)) +
  geom_bar(stat = "identity", width = 0.5) + 
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_hline(data = key, aes(yintercept = Max.value.agg), col = "grey", type = "dotted") +
  theme + 
  scale_fill_manual(values = ambition_palette) + guides(fill=FALSE) +
  facet_wrap(vars(Altname_f), ncol = 1, scales = "free_y") +
  xlab("Country") + ylab("Classroom-level VIND score") +
  labs(caption = "Error bars represent SE. Horizontal line shows maximum possible value.") + theme(plot.caption = element_text(hjust=0)) +
...
The participating teachers in each country/economy were asked to collect artefacts from four days of instruction on quadratic equations. These lessons were then rated along eleven components of teaching, aligned to the domains of quality teaching in the Global Teaching InSights observation system. All components were scored on a 3-point scale, except for Accuracy of Materials, which was scored on a 2-point scale.  
  
In addition to the artefacts from four instructional days, teachers were asked to submit the unit assessment that covered the topic of quadratic equations. The unit assessment was rated on a subset of the components: Connecting Mathematical Representations, Explicit Patterns and Generalisations, Real-World Connections, Using Multiple Mathematical Methods and Asking for Explanations. Unit assessments were rated separately and unit assessment ratings were not included in the construction of all classroom-level scores.
  
Classroom-level component scores were derived by averaging over raters and artefact sets. Score construction followed a two-step process:  
  
* average two raters' artefact-set level ratings to obtain artefact-set level component average scores  
* average the artefact-set averages to obtain a classroom-level component score  
...
```{r, echo = F, results='asis', fig.align='center', warning=FALSE}
# read in data dict
tmp <- read.delim(file.path(dir, "misc","teacher_file_variables_acomp_explanation.txt"), header = T)
...
The table below shows the descriptive statistics for all 11 artefact component scores, averaged across all teachers irrespective of country.  

```{r, echo = F, results='asis', fig.align='center', warning=FALSE}
# select relevant variables and save in a df
tmp <- teach[, grep("T_ID|COUNTRY|ACOMP", names(teach))]
...
# print in markdown
kbl(out, caption = "Descriptives of ACOMP* using whole dataset") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

Additionally, means and standard errors for each video component score were computed separately for each country and plotted below.  

```{r, echo = F, results='asis', fig.align='center', warning=FALSE, message = F, fig.height=9, fig.width=6}
# compute descriptives for each country
out <- apply(tmp[, c(-1, -2)], MARGIN = 2, function(x) psych::describeBy(x, group = tmp$COUNTRY, mat=TRUE, digits = 3))
...
plt <- ggplot(data = out, aes(x = group1, y = mean, fill = group1)) +
  geom_bar(stat = "identity", width = 0.5) + 
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_hline(data = key, aes(yintercept = Max.Rating), col = "grey", type = "dotted") +
  theme + 
  scale_fill_manual(values = ambition_palette) + guides(fill=FALSE) +
  facet_wrap(vars(Altname_f), ncol = 1) +
  xlab("Country") + ylab("Classroom-level VIND score") +
  labs(caption = "Error bars represent SE. Horizontal line shows maximum possible value.") + theme(plot.caption = element_text(hjust=0)) +
  ggtitle("Artefact component score seperately for each country")
...
```{r, echo = F, results='asis', fig.align='center', warning=FALSE, fig.width=7, fig.height=7}
# select relevant variables and save in a df
tmp <- teach[, grep("T_ID|COUNTRY|ACOMP", names(teach))]
...
<details>
<summary> <b> Appendix: Scatterplot and correlation matrix of artefact component scores for all countries separately </b> </summary>
...
ggpairs(tmp, columns = 3:ncol(tmp), mapping = aes(fill = COUNTRY, col = COUNTRY), diag = list(continuous = custom_dens), lower = list(continuous = costum_points),
        showStrips = T) + 
  scale_color_manual(values = ambition_palette) + scale_fill_manual(values = ambition_palette) +
  ggtitle("Scatterplot and correlation matrix of artefact component scores") + 
  theme
```
