---
title: "Measurement invariance (MI): Measures of instruction"
author: "Stef Meliss"
date: "`r Sys.Date()`"
...
# remove Shanghai
teach <- subset(teach, COUNTRY != "Shanghai")
```
...
## Video Component Scores (VCOMP)  
  
Shanghai has zero variance in the component VCOMP_CM1RT and was hence removed.

```{r, echo = F, results='asis'}

# determine unique variable names
vars <- c(
  # Assessment and Response to teachent Understanding
  "VCOMP_AR",
...
scales <- c(
  # Assessment and Response to teachent Understanding
  paste0(names(teach)[grep("VCOMP_AR", names(teach))],collapse = " + "),
  # Cognitive engagement
  paste0(names(teach)[grep("VCOMP_CE", names(teach))],collapse = " + "),
  # Classroom management
  paste0(names(teach)[grep("VCOMP_CM", names(teach))],collapse = " + "),
  # Discourse
  paste0(names(teach)[grep("VCOMP_DC", names(teach))],collapse = " + "),
  # Quality of Subject Matter 
  paste0(names(teach)[grep("VCOMP_QS", names(teach))],collapse = " + "),
  # Social-Emotional Support
  paste0(names(teach)[grep("VCOMP_SE", names(teach))],collapse = " + ")
)
...
To determine measurement invariance (MI) for the classroom-level video component ratings, CFA was applied. More specifically, for each of the six domains of teaching, we first defined a CFA model where all three components load on a single domain. All components were defined as continuous. 

> domain =~ component_1 + component_2 + component_3  
...
for (v in 1:length(vars)) {
  ...
  # fit CFA
  fit <-lavaan::cfa(model, data = teach) 

  # put summary in object
  tmp <- summary(fit, fit.measures = TRUE)
  
  cat("\n##### Fitting the model to all available data \n\n")
  ...
  # 
  # countries <- unique(teach$COUNTRY)
  # 
  ...
  #   tmp_fit <-lavaan::cfa(model, data = teach[teach$COUNTRY == countries[c], ])
  #   
  #   # put summary in object
  ...
  # config <- lavaan::cfa(model, data = teach, group = "COUNTRY")
  # 
  # # put summary in object
  ...
  # # metric invariance #
  # 
  # metric <-lavaan::cfa(model, data = teach, group = "COUNTRY", group.equal = c("loadings"))
  # scalar <-lavaan::cfa(model, data = teach, group = "COUNTRY", group.equal = c("loadings", "intercepts"))
  # 
  # # compare model fit
  ...
  #   kableExtra::column_spec(8, bold = TRUE) %>%
